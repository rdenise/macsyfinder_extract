...
class ClustersHandler(object):
    ...
    def __str__(self):
        ...
        for cluster in self.clusters:
            # BUG Manquait un +"\n" car sinon les str de cluster colapsais
        ...
        if not self.putative_system or force:
            ...
            if len(genes) == 1 and self.hits[0].gene.loner == False:
                ...
                if len(list(systems_compat.keys())) == 1:
                    ...
                    # NOTE Modif Rémi le if c'est Rémi et elif était avant un if mais maintenant c'est un elif donc remettre comme avant si bug
                    if sum(systems_compat.values()) == len(systems_compat) and len(cluster_compatible_systems) >= 1:
                        self._state = "clear"
                        cluster_compatible_systems.remove(self._putative_system)
                        self._compatible_systems = [self._putative_system] + cluster_compatible_systems
                        
                    elif len(cluster_compatible_systems) >= 1:
...
def build_clusters(hits, systems_to_detect, rep_info):
    ...
    if len(cur_cluster) > 1 or (len(cur_cluster) == 1 and cur_cluster.hits[0].gene.loner):
